...
def make_windowed_data(data, start, end, window_size = 1):
    ...
    ### YOUR CODE HERE (5-20 lines)
        ...
        for i in range(length):
            idx = i + window_size
            point = []
            for x in xrange(idx - window_size,idx + window_size + 1):
                point = point + sentence[x]
            windowed_data.append((point, labels[i]))                
    ### END YOUR CODE
...
class WindowModel(NERModel):
    ...
    def add_embedding(self):
        ...
        embeddings = tf.reshape(features, shape =(-1, self.config.n_window_features * self.config.embed_size))                                                                                                   
        ### END YOUR CODE
        return embeddings
    ...
    def add_prediction_op(self):
        ...
        nw_size = self.config.n_window_features 
        class_size = self.config.n_classes

        W = tf.get_variable("W", shape = (embed_size * nw_size, h_size), 
                            initializer = tf.contrib.layers.xavier_initializer()) 

        b1 = tf.get_variable("b1", shape = (h_size), 
                            initializer = tf.contrib.layers.xavier_initializer())

        U = tf.get_variable("U", shape = (h_size, class_size),
                            initializer = tf.contrib.layers.xavier_initializer())

        b2 = tf.get_variable("b2", shape = (class_size),
                            initializer = tf.contrib.layers.xavier_initializer())
