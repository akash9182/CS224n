...
\myhwtitle{2}{2 (g)}{\solutionsAuthor}
\bigskip

\begin{enumerate}
\item Momentum is like a heavy ball rolling down the hill. The ball follows the steepest path with inertia which acts as a smoother and an accelerator, dampening oscillations and causing the ball to travell through small humps and local minima.
\item SOmething
\end{enumerate}

\myhwtitle{3}{3 (a)}{\solutionsAuthor}
\bigskip
Since $y(t)$ is one-hot,
\begin{align}
    CE^{(t)}(\bm{y}^{(t)}, \hat{\bm{y}}^{(t)}) &= -\log{\hat{y}_j^{(t)}} \\
    =& \log{ \frac{1} {\hat{y}_j^{(t)}}\\ 
 \end{align} 
